...
class Evaluator(object):
    ...
    def cal_batch_lpips(self, preds, trues):
        ...
        if len(lpips_seq.shape) == 1:
            lpips_seq = np.expand_dims(lpips_seq, axis=1)  # 如果最后一批只有一个元素，做扩展
    ...
    def done(self):
        ...
        # print('mse')
        # print(self.losses['mse'])
        mses = np.nanmean(np.array(self.losses['mse']), axis=0)
        mass = np.nanmean(np.array(self.losses['mae']), axis=0)
        rmses = np.nanmean(np.array(self.losses['rmse']),axis=0)
        psnrs = np.nanmean(np.array(self.losses['psnr']), axis=0)
        ssims = np.nanmean(np.array(self.losses['ssim']), axis=0)
        crpss = np.nanmean(np.array(self.losses['crps']), axis=0)
        lpipss = np.nanmean(np.array(self.losses['lpips']), axis=0)
